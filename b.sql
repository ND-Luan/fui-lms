USE [LMS-LHBS]
GO
/****** Object:  StoredProcedure [dbo].[spAPI_KhoaCotDiem_Get]    Script Date: 2026-05-20 9:51:05 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    INNER JOIN dbo.tblMonHocLop mhl
        ...
        AND mhl.Enable             = 1
        AND mhl.NienKhoa           = @NienKhoa
    LEFT JOIN dbo.tblKhoaCotDiem kcd
        ON  kcd.MaCotDiem   = ct.MaCotDiem
        AND kcd.MonHocLopID = @MonHocLopID
        AND kcd.LopID       = @LopID          -- ← FIX: thêm lọc LopID
        AND kcd.Enable      = 1
    WHERE
    ...
    LEFT JOIN dbo.tblKhoaCotDiem kcd
        ON  kcd.MaCotDiem   = ct.MaCotDiem
        AND kcd.MonHocLopID = nlpc.MonHocLopID
        AND kcd.LopID       = @LopID          -- ← FIX: thêm lọc LopID
        AND kcd.Enable      = 1
    WHERE
        ct.MaNhomCotDiem = @MaNhomCotDiem
