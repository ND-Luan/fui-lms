...
;(function () {
    ...
    // ─── Lấy token (tự đăng ký SW) ───────────────────────────────────────────
    let _cachedToken = null;

    async function getDeviceToken(vapidKey = '') {
        if (_cachedToken) return _cachedToken;
        await ensureSDK();
        // Firebase tự đăng ký /firebase-messaging-sw.js nếu không truyền serviceWorkerRegistration
        _cachedToken = await _messaging.getToken({ vapidKey });
        return _cachedToken;
    }
    ...
    window.fcm = {
        ...
        async requestToken(vapidKey = '') {
            ...
            try {
                ...
                // SW chưa được deploy lên server → bỏ qua, không spam lỗi
                if (err?.message?.includes('MIME type') || err?.message?.includes('service-worker-registration') || err?.code === 'messaging/failed-service-worker-registration') {
                    console.warn('[FCM] Service Worker chưa được deploy tại /firebase-messaging-sw.js — bỏ qua FCM');
                } else {
