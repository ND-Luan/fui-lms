...
function confirm(obj) {
    $.confirm({
        ...
        columnClass: 'v-col-xs-12 v-col-sm-8 v-col-md-6 v-col-lg-4',
...
function showMessage(obj) {
    $.dialog({
        ...
        onClose: function () {
            if (obj.onclose) {
                runAction(obj.onclose, { item: obj.item });
            }
        },
    });
}
...
function openWindow(obj) {
    if (window === top) {
        setTimeout(function () {
            if(obj.id != undefined && app.config.globalProperties.v_OpenWindowList.find(item => item.id === obj.id) != undefined){
                console.log("Cửa sổ bị trùng tên: ", obj.id)
                return;
            }

            if (_.isString(obj)) obj = { url: obj };
            obj.uuid = "Window" + generateID();
            app.config.globalProperties.v_OpenWindowList.push(obj);
        }, 10);
    }
    else {
