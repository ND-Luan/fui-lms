<template>
	<div class="uc-rich-text-editor">
		<div v-if="$slots.toolbar || hasPlugin('ggb')" class="d-flex align-center mb-1">
			...
			<v-btn v-if="hasPlugin('ggb')" size="small" color="deep-purple-darken-1" variant="tonal" class="text-none mb-1" :loading="pluginLoading" @click="openPluginFile('ggb')">
				<v-icon start size="small">mdi-shape-polygon-plus</v-icon>Chèn hình GeoGebra (.ggb)
			</v-btn>
...
<script>
	...
	methods: {
		bindUploadAdapter() {
			const editorComponent = this.$refs.fEditor, editor = editorComponent && editorComponent.Editor
			if (!editor || !editor.plugins) { setTimeout(() => this.bindUploadAdapter(), 250); return }
			const repository = editor.plugins.get('FileRepository'); if (!repository) return
			...
			repository.createUploadAdapter = (loader) => ({
				upload: () => loader.file.then((file) => new Promise((resolve, reject) => {
					...
					manager.uploadLmsFile(file, { onComplete: (response) => { const item = response && response.Files && response.Files[0] ? response.Files[0] : response, baseUrl = (this.vueData.v_Set && this.vueData.v_Set.urlReturnFile) || 'https://file.lhbs.vn/lms', fileUrl = item && item.FILE_URL ? item.FILE_URL : '', url = fileUrl ? (fileUrl.indexOf('http') === 0 ? fileUrl : baseUrl + (fileUrl.indexOf('/') === 0 ? fileUrl : '/' + fileUrl)) : item && item.FILE_ID ? baseUrl + '/FileData/' + item.FILE_ID : ''; console.log('[uc-rich-text-editor] upload ảnh trả URL', { response: response, url: url }); if (url) resolve({ default: url }); else reject(new Error('FileData không trả về URL file')) }, onError: reject })
				})),
				abort: () => {},
			})
		},
		...
		openPluginFile(name) {
			...
			this.$refs.pluginFileInput.value = null; this.$refs.pluginFileInput.click()
		},
		handlePluginFile(event) {
			const file = event.target.files && event.target.files[0], manager = window.RichTextEditorPlugins
			if (!file || !manager || !manager.process) return
			this.pluginLoading = true
			manager.process('ggb', file, { insertImage: (url) => { this.editorContent = (this.editorContent || '') + '<p><img src="' + url + '" alt="GeoGebra PNG" style="max-width: 100%; height: auto; display: block; margin: 8px auto;" /></p>' }, onComplete: () => { this.pluginLoading = false }, onError: () => { this.pluginLoading = false } })
		},
