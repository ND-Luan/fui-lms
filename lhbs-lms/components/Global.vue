		...
		<v-responsive>
			<v-app theme="light">
				<v-layout v-if="hasNavigation">
					<GlobalNavigationDrawer :menuLeft="resolvedMenuLeft" :userInfo="resolvedUserInfo"
						:avatarUrl="resolvedAvatarUrl" :roleLabel="resolvedRoleLabel" :schoolName="schoolName"
						:version="version" :isMobile="isMobile" :currentUrl="activeModuleUrl" :rail="rail"
						v-model:mobileDrawer="mobileDrawer" @navigate="handleDrawerNavigate" />

					<v-main>
						<!-- Top Bar (Toolbar version) -->
						<v-toolbar v-if="hasNavigation" density="compact" color="white"
							style="position: sticky; top: 0; z-index: 1000">
							<v-btn icon="mdi-menu" v-if="!isMobile" @click="rail = !rail" variant="text" />
							<v-btn icon="mdi-menu" v-if="isMobile" @click="mobileDrawer = !mobileDrawer"
								variant="text" />
						...
						<div class="global-content-wrapper" style="height: calc(100dvh - 48px); width: 100%">
							<iframe v-show="resolvedModuleSrc" ref="moduleIframe" :src="resolvedModuleSrc"
...
<script>
	export default {
	    ...
	    computed: {
	        ...
	        resolvedMenuLeft() {
	            const raw = this.menuLeft || window.vueData?.menuLeft || this.RAW_MENU
	        ...
	        resolvedModuleSrc() {
	            if (this.activeModuleUrl && (this.vueData?.NienKhoa || this.vueData?.NienKhoaItem)) {
	    ...
	    mounted() {
	        ...
	        // Sync NienKhoa via postMessage
	        this.$watch(
	            () => window.vueData?.NienKhoaItem,
	            (newVal) => {
	                this.sendMessageToIframe('SCHOOL_YEAR_CHANGED', {
	                    schoolYear: newVal?.NienKhoa,
	                    schoolYearItem: newVal,
	                })
	            },
	            { deep: true },
	        )
	    ...
	    methods: {
	        handleDrawerNavigate(url) {
	            if (!url || this.activeModuleUrl === url) return
	            this.activeModuleUrl = url
	        ...
	        onIframeLoad() {
	            this.sendMessageToIframe('INIT', {
	                schoolYear: window.vueData?.NienKhoa,
	                schoolYearItem: window.vueData?.NienKhoaItem,
	            })
	        },
