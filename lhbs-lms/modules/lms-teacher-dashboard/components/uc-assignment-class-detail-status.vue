	...
	<v-container class="py-6" fluid>
		...
		<v-row class="mb-4" align="center">
			...
			<v-col cols="12" md="6" class="d-flex justify-end gap-2">
				<v-select v-model="selectedClassId" :items="classes" item-title="name" item-value="id" label="Lớp"
					variant="solo" density="comfortable" hide-details style="max-width: 180px" />
				<v-select v-model="selectedAssignmentId" :items="assignments" item-title="title" item-value="id"
					label="Bài tập" variant="solo" density="comfortable" hide-details style="max-width: 220px" />
				<v-text-field v-model="search" label="Tìm kiếm" prepend-inner-icon="mdi-magnify" variant="solo"
					density="comfortable" hide-details style="max-width: 240px" />
			</v-col>
		</v-row>
		...
		<v-card rounded="xl" elevation="2">
			<v-data-table :headers="headers" :items="filteredRows" :items-per-page="10" density="comfortable"
				class="rounded-xl">
				<template #item.status="{ item }">
					<v-chip :color="item.status === 'Đã nộp' ? 'success' : 'warning'" size="x-small" variant="flat">{{
						item.status }}</v-chip>
				</template>
				...
				<template #item.actions="{ item }">
					<v-btn size="x-small" color="primary" variant="outlined" @click="openGradeDialog(item)">Chấm bài</v-btn>
				</template>
			</v-data-table>
		</v-card>
			...
			<v-card rounded="xl">
				...
				<v-card-text>
					...
					<v-row class="mt-4" dense>
						<v-col cols="12" sm="6">
							<v-text-field v-model.number="gradeDialog.score" :max="currentAssignment.maxScore" :min="0"
								type="number" label="Điểm" variant="outlined" density="comfortable" />
						</v-col>
						<v-col cols="12" sm="6">
							<v-text-field v-model="gradeDialog.bonus" label="Điểm cộng (tuỳ chọn)" type="number"
								variant="outlined" density="comfortable" />
						</v-col>
						<v-col cols="12">
							<v-textarea v-model="gradeDialog.comment" label="Nhận xét" rows="3" variant="outlined"
								density="comfortable" />
						</v-col>
					</v-row>
				</v-card-text>
