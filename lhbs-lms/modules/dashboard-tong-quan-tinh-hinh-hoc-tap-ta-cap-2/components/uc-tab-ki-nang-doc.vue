<template>
    <v-card>
        <v-card-title class="text-primary">
            Bảng tỉ lệ điểm kĩ năng đọc theo khối
        </v-card-title>
        <v-card-text>
            <v-table>
                <thead>
...
export default {
    ...
    data() {
        return {
            ...
            subHeader: [
                { title: "Chưa đạt", key: "Doc_CD_PCT_GK" },
                { title: "Đạt", key: "Doc_D_PCT_GK" },
                { title: "Vượt đạt", key: "Doc_VYC_PCT_GK" },
                { title: "Chưa đạt", key: "Doc_CD_PCT_CK" },
                { title: "Đạt", key: "Doc_D_PCT_CK" },
                { title: "Vượt đạt", key: "Doc_VYC_PCT_CK" },
            ],
            dataSkills: [],
            Chart_TiLe_TheoLop: {
    ...
    async mounted() {
        await this.handleData(['KhoiID', 'TongHocSinh', 'Doc'])
    ...
    methods: {
        async handleData(keysGet) {
			...
			data.forEach(khoi => {
                let khoiCK = vueData.Dashboard_Cambridge_Theo_KyNang_CK.find(x => x.TenNhom === khoi.TenNhom)
                if (khoiCK) {
                    //Phân rã key trong Objet
            ...
            const serieChartTiLe_TheoLops = [
                {
                    ...
                    data: this.dataSkills.map(data => data.Doc_VYC_PCT_GK)
                },
                {
                    name: 'Vượt yêu cầu - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.Doc_VYC_PCT_CK)
                },
                {
                    name: 'Đạt yêu cầu - Giữa kì',
                    group: 'S1_Mid_Total_Conv',
                    data: this.dataSkills.map(data => data.Doc_D_PCT_GK)
                },
                {
                    name: 'Đạt yêu cầu - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.Doc_D_PCT_CK)
                },
                {
                    name: 'Chưa đạt - Giữa kì',
                    group: 'S1_Mid_Total_Conv',
                    data: this.dataSkills.map(data => data.Doc_CD_PCT_GK)
                },
                {
                    name: 'Chưa đạt - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.Doc_CD_PCT_CK)
                }
            ]
