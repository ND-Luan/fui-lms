<template>
	<v-card>
		<v-card-title class="text-primary">
			Bảng tỉ lệ điểm trung bình theo khối
		</v-card-title>
		<v-card-text>
			<v-table>
				<thead>
...
<script>
    ...
    data() {
        return {
            ...
            subHeader: [
                { title: "Chưa đạt", key: "TrungBinh_CD_PCT_GK" },
                { title: "Đạt", key: "TrungBinh_D_PCT_GK" },
                { title: "Vượt đạt", key: "TrungBinh_VYC_PCT_GK" },
                { title: "Chưa đạt", key: "TrungBinh_CD_PCT_CK" },
                { title: "Đạt", key: "TrungBinh_D_PCT_CK" },
                { title: "Vượt đạt", key: "TrungBinh_VYC_PCT_CK" },
            ],
            dataSkills: [],
            Chart_TiLe_TheoLop: {
    ...
    async mounted() {
        await this.handleData(['TenNhom', 'TongHocSinh', 'TrungBinh'])
            ...
            const serieChartTiLe_TheoLops = [
                {
                    ...
                    data: this.dataSkills.map(data => data.TrungBinh_VYC_PCT_GK)
                },
                {
                    name: 'Vượt yêu cầu - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.TrungBinh_VYC_PCT_CK)
                },
                {
                    name: 'Đạt yêu cầu - Giữa kì',
                    group: 'S1_Mid_Total_Conv',
                    data: this.dataSkills.map(data => data.TrungBinh_D_PCT_GK)
                },
                {
                    name: 'Đạt yêu cầu - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.TrungBinh_D_PCT_CK)
                },
                {
                    name: 'Chưa đạt - Giữa kì',
                    group: 'S1_Mid_Total_Conv',
                    data: this.dataSkills.map(data => data.TrungBinh_CD_PCT_GK)
                },
                {
                    name: 'Chưa đạt - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.TrungBinh_CD_PCT_CK)
                }
            ]
