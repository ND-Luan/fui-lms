<template>
	<v-card>
		<v-card-title class="text-primary">
			Bảng tỉ lệ điểm kĩ năng viết theo khối
		</v-card-title>
		<v-card-text>
			<v-table>
				<thead>
...
<script>
    ...
    data() {
        return {
            ...
            subHeader: [
                { title: "Chưa đạt", key: "Viet_CD_PCT_GK" },
                { title: "Đạt", key: "Viet_D_PCT_GK" },
                { title: "Vượt đạt", key: "Viet_VYC_PCT_GK" },
                { title: "Chưa đạt", key: "Viet_CD_PCT_CK" },
                { title: "Đạt", key: "Viet_D_PCT_CK" },
                { title: "Vượt đạt", key: "Viet_VYC_PCT_CK" },
            ],
            dataSkills: [],
            Chart_TiLe_TheoLop: {
    ...
    async mounted() {
        await this.handleData(['TenNhom', 'TongHocSinh', 'Viet'])
            ...
            const serieChartTiLe_TheoLops = [
                {
                    ...
                    data: this.dataSkills.map(data => data.Viet_VYC_PCT_GK)
                },
                {
                    name: 'Vượt yêu cầu - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.Viet_VYC_PCT_CK)
                },
                {
                    name: 'Đạt yêu cầu - Giữa kì',
                    group: 'S1_Mid_Total_Conv',
                    data: this.dataSkills.map(data => data.Viet_D_PCT_GK)
                },
                {
                    name: 'Đạt yêu cầu - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.Viet_D_PCT_CK)
                },
                {
                    name: 'Chưa đạt - Giữa kì',
                    group: 'S1_Mid_Total_Conv',
                    data: this.dataSkills.map(data => data.Viet_CD_PCT_GK)
                },
                {
                    name: 'Chưa đạt - Cuối kì',
                    group: 'S1_Final_Total_Conv',
                    data: this.dataSkills.map(data => data.Viet_CD_PCT_CK)
                }
            ]
