			...
			<v-card-text>
				...
				<v-form ref="form" fast-fail>
					<v-row>
						...
						<v-col cols="12">
							<v-text-field v-model="TenKyNang" :rules="rules" label="Tên kĩ năng" variant="filled"
								:clearable="false" />
						</v-col>
						<v-col cols="12">
							<v-textarea v-model="MoTaKyNang" :rules="rules" label="Mô tả" variant="filled"
								:clearable="false" />
						</v-col>
					</v-row>
				</v-form>
...
export default {
	...
	methods: {
		...
		async onSubmit() {
			...
			if (valid) {
				await this.submit(this.TenKyNang.trim(), this.MoTaKyNang.trim());
				this.close();
			}
		}
	},
