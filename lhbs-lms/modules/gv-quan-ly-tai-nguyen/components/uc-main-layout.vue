<template>
	<Global>
		...
		<v-data-table
			...
			<template #item.actions="{ item }">
				<div class="d-flex ga-1">
					<v-btn icon="mdi-pencil" size="small" variant="text" color="primary"
						@click="openEdit(item)" />
					<v-btn icon="mdi-file-edit-outline" size="small" variant="text" color="secondary"
			...
			<template #no-data>
				<div class="d-flex flex-column align-center justify-center py-12 text-medium-emphasis">
					<v-icon size="48" class="mb-3 opacity-40">mdi-file-search-outline</v-icon>
					<p class="text-body-2">Chưa có tài nguyên nào. Thay đổi bộ lọc hoặc thêm mới.</p>
				</div>
			</template>
		</v-data-table>
		...
		<uc-dialog
			v-model="dialog"
			:title="isEditMode ? 'Cập nhật tài nguyên' : 'Thêm tài nguyên mới'"
			done-text="Lưu"
			:width="560"
			@onSubmit="onSave">
			...
			<v-row dense>
				<v-col cols="12">
					<v-text-field v-model="editedItem.TenHocLieu" label="Tên tài nguyên"
						:rules="[v => !!v || 'Bắt buộc']" />
				</v-col>
				<v-col cols="12" sm="6">
...
<script>
	...
	data() {
		return {
			...
			headers: [
				{ title: 'Tên tài nguyên', value: 'TenHocLieu', sortable: true },
	...
	methods: {
		getEmptyItem() {
			return {
				...
				ThumbnailURL: null,
				TinhTrang: null,
				Loai: this.resourceType,
			}
		},

		async getTaiNguyen() {
			this.taiNguyenList = await fetchPromise('lms/FP_HocLieu_GetAll', {
				PageNumber: 1,
				PageSize: 999,
		...
		async openEdit(item) {
			...
			this.editedItem = { ...this.getEmptyItem(), ...item, Loai: this.resourceType }
			this.DSMonDialog = []
		...
		async onSave() {
			if (!this.editedItem.TenHocLieu) {
				this.snackbarRef.value.show({ message: 'Vui lòng nhập tên tài nguyên', color: 'warning' })
				return
			}

			const ok = await this.confirmRef.value.show({
				title: this.isEditMode ? 'Xác nhận cập nhật tài nguyên?' : 'Xác nhận thêm tài nguyên?',
			})
			if (!ok) return
			...
			const message = this.isEditMode ? 'Cập nhật tài nguyên thành công' : 'Thêm tài nguyên thành công'
		...
		async onDelete(item) {
			const ok = await this.confirmRef.value.show({
				title: `Xóa tài nguyên "${item.TenHocLieu}"?`,
			})
			if (!ok) return

			ajaxCALL('lms/FP_HocLieu_Delete', { HocLieuID: item.HocLieuID }, () => {
				this.snackbarRef.value.show({ message: 'Xóa tài nguyên thành công', color: 'success' })
				this.getTaiNguyen()
			})
		},

		goToSoanThao(item) {
			openWindow({ title: `Soạn thảo: ${item.TenHocLieu}`, url: `/soan-hoc-lieu-v2?HocLieuID=${item.HocLieuID}&Loai=${this.resourceType}` })
		},
	},
	}
</script>