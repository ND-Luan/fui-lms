	...
	methods: {
				...
				if (window.UploadManager && window.UploadManager.uploadLmsFile) {
					window.UploadManager.uploadLmsFile(pngFile, {
						onComplete: (res) => {
							...
							if (fileObj?.FILE_URL) {
								imgUrl = fileObj.FILE_URL.startsWith('http') ? fileObj.FILE_URL : (baseUrl + fileObj.FILE_URL)
							} else if (fileObj?.FILE_ID) {
								imgUrl = `${baseUrl}/FileData/${fileObj.FILE_ID}`
							} else if (res?.url) {
								imgUrl = res.url
							...
							if (imgUrl) {
								...
								this.showNotification('Lỗi: Server không trả về đường dẫn ảnh hợp lệ', 'error')
							}
						},
						onError: (err) => {
							this.isProcessing = false
					...
					// Fallback nếu không có UploadManager: dùng Base64 trực tiếp
		...
		detectAndHandleDeletedImages(oldHtml, newHtml) {
			...
			deletedSrcs.forEach(src => {
				// Trích xuất fileId từ URL kiểu /FileData/fileId hoặc FILE_ID
				const match = src.match(/\/FileData\/([^\/\?]+)/i)
				const fileId = match ? match[1] : null

				if (fileId && typeof UploadManager !== 'undefined' && UploadManager.deleteLmsFile) {
					console.log('🗑️ Xóa file trên server do người dùng xóa ảnh khỏi editor:', fileId)
					UploadManager.deleteLmsFile(fileId, {
						onComplete: () => {
							this.showNotification(`Đã xóa file ảnh (${fileId}) khỏi server!`, 'info')
						},
						onError: (err) => {
							console.warn('Lỗi khi xóa file trên server:', err)
						}
					})
				}
			})
