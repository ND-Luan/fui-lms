...
- **Quy tắc cấp quyền SQL**: Tất cả Stored Procedure mới hoặc cập nhật bắt buộc cấp đủ 3 quyền cho `[lmslhbs]`: `GRANT EXECUTE ON OBJECT::[dbo].[spName] TO [lmslhbs];`, `GRANT VIEW DEFINITION ON OBJECT::[dbo].[spName] TO [lmslhbs];` và `GRANT CONTROL ON OBJECT::[dbo].[spName] TO [lmslhbs];` (tuyệt đối không `GRANT` cho `[public]`).
