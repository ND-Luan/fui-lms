<template>
	<div class="lms-dashboard-root teacher-dashboard" ref="containerPage">
		<!-- TOPBAR -->
		<v-toolbar border density="compact" class="bg-white border-sm" style="position:sticky;top:0;z-index:50;flex-shrink:0;">
			<template #title>
				<div class="d-flex align-center">
					...
					<div class="lms-breadcrumb ms-3">
						<template v-for="(bc, i) in cascadeBreadcrumb" :key="i">
							...
							<span class="bc-chip"
								:class="bc.active ? 'bc-active' : bc.done ? 'bc-done' : 'bc-lock'"
			...
			<div class="lms-cascade" ref="tourCascade">
				...
				<div class="lms-tier">
					...
					<div class="lms-tier-body">
						...
						<template v-else>
						<div v-for="k in DSKhoiCascade" :key="k.KhoiID"
							class="lms-t-item" :class="{ active: cascadeKhoiID === k.KhoiID }"
							@click="pickKhoi(k.KhoiID)">
				...
				<div class="lms-tier">
					<div class="lms-tier-hd">
						...
						<button v-if="cascadeKhoiID" class="lms-tier-reset" @click="resetCascadeFrom(1)">đổi khối</button>
					</div>
					<div class="lms-tier-body">
						<div v-if="!cascadeKhoiID" class="lms-tier-locked">
							<v-icon size="13">mdi-lock-outline</v-icon> Chọn khối trước
						</div>
						<div v-for="m in DSMonCascade" v-else :key="m.MonHocName"
							class="lms-t-item" :class="{ active: cascadeMonHocName === m.MonHocName }"
							@click="pickMon(m.MonHocName)">
							<span class="lms-t-dot" :style="{ background: m.dot }"></span>
							<span class="lms-t-label">{{ m.MonHocName === 'Ngoại ngữ' ? 'English' : m.MonHocName }}</span>
				...
				<div class="lms-tier lms-tier-flex">
					<div class="lms-tier-hd">
						...
						<button v-if="cascadeMonHocName" class="lms-tier-reset" @click="resetCascadeFrom(2)">đổi môn</button>
					</div>
					<div class="lms-tier-body">
					...
					<template v-else>
						<div v-for="w in DSTuanCascade" :key="w.TuanHocID"
							class="lms-t-item lms-t-item--week"
							:class="{ active: cascadeTuanID === w.TuanHocID }"
							@click="pickTuan(w.TuanHocID)">
						...
						<div v-if="currentTuanFallback"
							...
							<v-icon v-if="cascadeTuanID === currentTuanFallback.TuanHocID" size="14" color="teal">mdi-check</v-icon>
							<span v-else class="lms-t-arrow">›</span>
						</div>
					</template>
					</div>
			...
			<div class="lms-main">
				...
				<template v-if="!cascadeTuanID">
					<div class="lms-empty-state">
						...
						<p class="text-body-2 font-weight-medium text-medium-emphasis">
							{{ !cascadeKhoiID ? 'Chọn khối để bắt đầu' : !cascadeMonHocName ? 'Chọn môn học' : 'Chọn tuần học' }}
						</p>
				...
				<template v-else>
					...
					<div class="lms-table-card" ref="tourClassTable">
						...
						<div v-if="centerClasses.length === 0" class="text-center pa-8 text-disabled d-flex justify-center align-center flex-column">
							...
							<div v-for="classItem in centerClasses" :key="classItem.LopID">
								<template v-if="classItem.LopID !== -1">
									...
									<div class="lms-class-row">
										...
										<v-btn icon size="small" variant="text" color="purple" @click="xemTinhTrang(classItem)">
											<v-icon>mdi-table-eye</v-icon>
											<v-tooltip activator="parent" location="top">{{ $t('message.ViewGradebook') }}</v-tooltip>
										</v-btn>
									</div>
									<!-- Assignment rows -->
		...
		<uc-btn-with-dialog-add-bt v-model:isOpen="isShowModalAddNoiDung" v-if="isShowModalAddNoiDung" :KhoiItem :tuanHocID="cascadeTuanID" :defaultType="defaultType" />
		...
		<uc-tour-overlay
			...
			:body="tourSteps[tourStep] && tourSteps[tourStep].body"
