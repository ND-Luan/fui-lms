<template>
	<div class="teacher-dashboard pa-2 h-100">
		<!-- Header và Tabs không đổi -->
		<div class="d-flex justify-space-between align-center mb-2">
			<h1 class="text-h6">Bảng điều khiển Giảng dạy</h1>
		</div>
		<div class="mb-2">
			<h2 class="widget-title mb-2 d-flex align-center">
				<v-icon color="orange" class="me-2">mdi-target</v-icon>Bài tập cần chú ý
			</h2>
			<div v-if="!focusTasks || focusTasks.length === 0" class="text-center pa-5 grey--text rounded border">
				<p class="mb-0">Không có bài tập nào cần chú ý ngay. Rất tốt!</p>
			</div>
			<v-row v-else>
				<v-col v-for="task in focusTasks" :key="task.AssignToClassID" cols="12" lg="4" md="6">
		...
		<v-window v-model="activeTab">
			...
			<v-window-item value="classes">
				<v-row dense>
					<v-col cols="12" md="8">
						<div v-if="!teachingGroups || teachingGroups.length === 0" class="text-center pa-10 border rounded">
							<p class="text-grey">Bạn chưa được phân công giảng dạy lớp nào.</p>
						</div>
						<uc-teaching-group-card v-for="group in teachingGroups" :key="group.KhoiID + '-' + group.MonHocID"
							:group="group" @view-class="$emit('view-class', $event)" @grade-class="$emit('grade-class', $event)"
			...
			<v-window-item value="library">
				<v-row dense>
					<div v-if="!contentLibrary || contentLibrary.length === 0" class="text-center pa-10 border rounded">
