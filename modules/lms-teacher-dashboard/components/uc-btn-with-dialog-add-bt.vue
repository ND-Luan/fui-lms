	...
	<v-dialog v-model="isOpen" max-width="500">
		<v-card>
			<v-card-title class="d-flex align-center">
				Tạo nội dung - {{ KhoiItem?.TenKhoiHoc }}
				<v-spacer />
					...
					<v-row>
						<v-col cols="12">
							...
							<div class="d-flex align-center">
								...
								<div>
									<div class="text-overline">{{ KhoiItem?.TenKhoi }}</div>
									<div class="text-h6">{{ KhoiItem?.MonHocName }}</div>
								</div>
							</div>
						</v-col>
						<v-col cols="12">
						...
						<v-col cols="12">
							<v-text-field v-model="form.Chuong" label="Chương"/>
						</v-col>
						<v-col cols="12">
							<v-text-field v-model="form.Title" label="Tiêu đề" required
			...
			<v-card-actions>
				<v-spacer />
				<v-btn @click="CloseModal" text="Đóng" />
				<v-btn @click="handleSubmit()" color="primary" variant="elevated" text="Tạo" />
			</v-card-actions>
		</v-card>
	</v-dialog>
</template>
