<template>
	<v-dialog v-model="isOpen" width="1000" persistent>
		<v-card>
			...
			<v-card-text class="px-2 py-3">
				...
				<v-form ref="formData">
					<v-row dense>
						...
						<v-col cols="12" md="12">
							<v-checkbox v-model="formData.IsPublic" label="Công khai" hide-details="auto" dense />
						</v-col>
					</v-row>
				</v-form>
			...
			<div class="mt-3" v-if="selectedLibery?.AssignedClassNames?.length > 0">
				<v-data-table :headers="headersAssignedClass" :items="assignedClassList" :hide-default-footer="true">
					...
					<template v-slot:item.actions="{ item }">
						<v-btn size="small" variant="tonal" color="primary" @click="editGiaoBaiTapDialog(item)">Sửa
							nội dung</v-btn>
					</template>
				</v-data-table>
			</div>
	...
	data() {
		return {
			...
			headersAssignedClass: [
				{ title: 'Lớp', value: 'TenLopHoacNhom', key: 'TenLopHoacNhom', sortable: false },
				{ title: 'Hạn nộp', value: 'DueDate', align: 'center', key: 'DueDate', sortable: false },
				{ title: 'Giới hạn lần nộp', value: 'LimitAssigned', align: 'center', key: 'LimitAssigned', sortable: false },
				{ title: 'Điểm tối đa', value: 'MaxScore', align: 'center', key: 'MaxScore', sortable: false },
				{ title: 'Thao tác', value: 'actions', align: 'center', sortable: false, key: 'actions', sortable: false },
			],
		}
	},
	computed: {
	...
	methods: {
		...
		async saveEditAssign(item) {
			...
			if (index != null) {
				...
				const payload = JSON.stringify([this.editData[index]])

				ajaxCALL("/lms/EL_Teacher_AssignToClasses_CLASS", {
					AssignmentID: this.editData[index].AssignmentID,
					JsonClassItems: payload
				}, (res) => {
		...
		saveEditLimitAssigned(item) {
			let index = this.getIndexToEdit(item)
			if (index != null) {
				const payload = JSON.stringify([this.editDataLimitAssigned[index]])
				console.log('[this.editDataLimitAssigned[index]]', this.editDataLimitAssigned[index])
				ajaxCALL("/lms/EL_Teacher_AssignToClasses_CLASS", {
					AssignmentID: this.editDataLimitAssigned[index].AssignmentID,
					JsonClassItems: payload
				}, (res) => {
