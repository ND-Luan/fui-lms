			...
			<v-row align="center">
					...
					<v-row dense>
						<v-col cols="4" sm="2">
							<v-select v-model="selectedLopID" :items="lopList" item-title="TenLop" item-value="LopID"
								label="Lớp" variant="outlined" density="compact" hide-details />
						</v-col>
						<v-col cols="8" sm="2">
							<v-select v-model="selectedMonHocID" :items="monHocList" item-title="MonHocName"
								item-value="MonHocID" label="Môn học" variant="outlined" density="compact" hide-details
						...
						<v-col cols="12" sm="8" class="d-flex ga-2">
							...
							<v-btn variant="outlined" color="primary"
								@click="() => { this.loadAllDataForAssignment(this.selectedAssignToClassID) }"><v-icon
									size="small">mdi-refresh</v-icon>Làm mới</v-btn>
						</v-col>
					</v-row>
				</v-col>
				...
				<v-col cols="12" md="4" class="d-flex align-center justify-space-around ">
					...
					<div class="text-center">
						...
						<div class="text-caption">trên thang {{ stats.MaxScore || '-' }}</div>
					</div>
				</v-col>
			</v-row>
		</v-card-text>
	...
	<v-dialog v-model="isOpen" fullscreen>
		<v-card class="ChamBaiDialog" id="ChamBaiDialog" style=" position: sticky; top: 0 ;z-index:9999">
			<v-card-title class="d-flex bg-primary" style="max-height: 48px;">
				<span class="text-white">Chấm bài</span><v-spacer></v-spacer>
	...
	watch: {
		...
		selectedAssignToClassID(newId, oldId) {
			if (newId && newId !== oldId) {
				...
				console.log('vueData.assigntoclassid', vueData.AssignToClassID_FromURL)
		...
		isOpen(val) {
			console.log('document.getElementsByClassName("v-toolbar")', document.getElementsByClassName("v-toolbar"))
			if (val) {
