		...
		<v-card>
			...
			<v-card-text class="px-2 py-3">
				...
				<div class="d-flex mb-4 ga-2">
					<v-chip color="orange">{{ $t('message.Grade') }} {{ formData.KhoiID }}</v-chip>
			...
			<div class="mt-3" v-if="selectedLibery?.AssignedClassNames?.length > 0">
				<v-data-table :headers="headersAssignedClass" :items="assignedClassList" :hide-default-footer="true">
					...
					<template v-slot:item.actions="{ item }">
						<v-btn size="small" variant="tonal" color="primary" @click="editGiaoBaiTapDialog(item)">{{
							$t('message.EditContent') }}</v-btn>
					</template>
				</v-data-table>
			</div>
	...
	data() {
		return {
			...
			headersAssignedClass: [
				...
				{ title: this.$i18n.locale == 'en' ? 'Action' : 'Thao tác', value: 'actions', align: 'center', sortable: false, key: 'actions', sortable: false },
			],
		}
	},
	computed: {
