	...
	<v-navigation-drawer v-model="isOpen" temporary location="end" width="400">
		...
		<div class="pa-2 " style="display: flex;flex-direction: row; flex-wrap: wrap; gap: 4px;">
			...
			<v-btn v-for="(item, index) in DSMonHoc" :key="index" size="small" :value="item" variant="tonal"
				...
				<span class="text-subtitle-2" style="font-size: .775rem !important;">{{ item == 'Ngoại ngữ' ?
					($i18n.locale ==
						'en' ? 'English' : item) : item }}</span>
			</v-btn>
	...
	methods: {
		...
		statusInfo(item) {
			const statusMap = {
				...
				3: { text: this.$i18n.locale == 'en' ? `Assigned ${item.AssignedClassCount} class` : `Đã giao ${item.AssignedClassCount} lớp`, color: 'success' }
			};
