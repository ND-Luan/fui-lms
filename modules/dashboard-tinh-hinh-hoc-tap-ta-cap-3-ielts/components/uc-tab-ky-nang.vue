				...
				<v-col>
					<v-card :flat="false">
						<v-card-title class="text-primary">
							Biểu đồ điểm (Listening, Reading, Writing, Speaking) kĩ năng theo cấp
						</v-card-title>
						<v-card-text>
...
export default {
	...
	methods: {
		...
		async onLoadDSMaNhomCotDiem(KhoiID) {
			...
			const res2 = await this.ajaxPromise('lms/NhomCauTrucDiem_Get_ByMonHocID')
			if (res2) {
				this.DSNhomDiem = res2.data.filter(x => {
					const ten = x.TenNhomCotDiem_VI.toLowerCase();
					return ten.includes('ielts') && (ten.includes('giữa hk') || ten.includes('cuối hk'))
				});
		...
		onLoadChart({ NienKhoa, CapID, MonHocID, MaNhomDiem, KhoiID }) {
			return new Promise(resolve => {
				ajaxCALL('lms/DashboardDiemKyNang_IELTS_Get',
					{
						NienKhoa,
						CapID,
						MonHocID,
		...
		convertChartLineTongDiem_KyNang_TheoKhoi(rawData) {
			const stats_GK = this.processData(rawData.filter(x => (x.MaNhomCotDiem.includes('S1_Mid')) || (x.MaNhomCotDiem.includes('S2_Mid'))), 'TenLop');
			const classes_GK = Object.keys(stats_GK);

			const stats_CK = this.processData(rawData.filter(x => (x.MaNhomCotDiem.includes('S1_Final')) || (x.MaNhomCotDiem.includes('S2_Final'))), 'TenLop');
			...
			if (Object.keys(stats_GK).length > 0) {
				console.log('form', this.form.MaNhomCotDiem)
				seriesData.push({
					name: 'Trung bình (Mean) - GK' + `${this.form.MaNhomCotDiem[0].includes('S1_Mid') ? "1" : "2"}`,
					type: 'line',
					data: classes_GK.map(lop => stats_GK[lop].mean)
				})
				seriesData.push({
					name: 'Trung vị (Median) - GK' + `${this.form.MaNhomCotDiem[0].includes('S1_Mid') ? "1" : "2"}`,
					type: 'line',
					data: classes_GK.map(lop => stats_GK[lop].median)
				})
				seriesData.push({
					name: 'Độ lệch chuẩn (Std Dev) - GK' + `${this.form.MaNhomCotDiem[0].includes('S1_Mid') ? "1" : "2"}`,
					type: 'line',
			...
			if (Object.keys(stats_CK).length > 0) {
				seriesData.push({
					name: 'Trung bình (Mean) - CK' + `${this.form.MaNhomCotDiem[0].includes('S1_Final') ? "1" : "2"}`,
					type: 'line',
					data: classes_CK.map(lop => stats_CK[lop].mean)
				})
				seriesData.push({
					name: 'Trung vị (Median) - CK' + `${this.form.MaNhomCotDiem[0].includes('S1_Final') ? "1" : "2"}`,
					type: 'line',
					data: classes_CK.map(lop => stats_CK[lop].median)
				})
				seriesData.push({
					name: 'Độ lệch chuẩn (Std Dev) - CK' + `${this.form.MaNhomCotDiem[0].includes('S1_Final') ? "1" : "2"}`,
					type: 'line',
			...
			this.Chart_TongDiem_KyNang_TheoKhoi = {
				...
				title: {
					text: 'Mean, Median, and Standard Deviation of Total Scores (Listening, Reading, Writing, Speaking) by Grade',
					align: 'center'
				},
				grid: {
					row: {
		...
		convertChartLine_KyNang_TheoKhoi(_rawData) {
			...
			for (const khoiID of uniqueKhoiID) {
				...
				const stats_GK = this.processData(_rawData.filter(x => (x.MaNhomCotDiem.includes('S1_Mid') && x.KhoiID === this.form.KhoiID) || (x.MaNhomCotDiem.includes('S2_Mid') && x.KhoiID === this.form.KhoiID)), 'TenCotDiem_EN');
				const classes_GK = Object.keys(stats_GK);

				const stats_CK = this.processData(_rawData.filter(x => (x.MaNhomCotDiem.includes('S1_Final') && x.KhoiID === this.form.KhoiID) || (x.MaNhomCotDiem.includes('S2_Final') && x.KhoiID === this.form.KhoiID)), 'TenCotDiem_EN');
				const classes_CK = Object.keys(stats_CK);
		...
		convertChartLine_KyNang_TheoLop(_rawData) {
			...
			const stats_Listening_GK = this.processData(sortDataListening.filter(x => (x.MaNhomCotDiem.includes('S1_Mid')) || (x.MaNhomCotDiem.includes('S2_Mid'))), 'TenLop');
			// const stats_Language_GK = this.processData(sortDataLanguage.filter(x=> (x.MaNhomCotDiem.includes('S1_Mid'))|| (x.MaNhomCotDiem.includes('S2_Mid'))), 'TenLop');
			const stats_Reading_GK = this.processData(sortDataReading.filter(x => (x.MaNhomCotDiem.includes('S1_Mid')) || (x.MaNhomCotDiem.includes('S2_Mid'))), 'TenLop');
			const stats_Writing_GK = this.processData(sortDataWriting.filter(x => (x.MaNhomCotDiem.includes('S1_Mid')) || (x.MaNhomCotDiem.includes('S2_Mid'))), 'TenLop');
			const stats_Speaking_GK = this.processData(sortDataSpeaking.filter(x => (x.MaNhomCotDiem.includes('S1_Mid')) || (x.MaNhomCotDiem.includes('S2_Mid'))), 'TenLop');

			const stats_Listening_CK = this.processData(sortDataListening.filter(x => (x.MaNhomCotDiem.includes('S1_Final')) || (x.MaNhomCotDiem.includes('S2_Final'))), 'TenLop');
			// const stats_Language_CK = this.processData(sortDataLanguage.filter(x=> (x.MaNhomCotDiem.includes('S1_Final')) || (x.MaNhomCotDiem.includes('S2_Final'))), 'TenLop');
			const stats_Reading_CK = this.processData(sortDataReading.filter(x => (x.MaNhomCotDiem.includes('S1_Final')) || (x.MaNhomCotDiem.includes('S2_Final'))), 'TenLop');
			const stats_Writing_CK = this.processData(sortDataWriting.filter(x => (x.MaNhomCotDiem.includes('S1_Final')) || (x.MaNhomCotDiem.includes('S2_Final'))), 'TenLop');
			const stats_Speaking_CK = this.processData(sortDataSpeaking.filter(x => (x.MaNhomCotDiem.includes('S1_Final')) || (x.MaNhomCotDiem.includes('S2_Final'))), 'TenLop');
			...
			this.List_Chart_KyNang_TheoLop = [kyNang.Listening, kyNang.Reading, kyNang.Writing, kyNang.Speaking]
		},
		calculateKDE,
		linspace,
