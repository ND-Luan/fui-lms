<template>
	<div>
		<v-divider></v-divider>
		<v-list class="pt-0" style="overflow: auto; height: calc(100dvh - 221px);">
				...
				<keep-alive>
					<v-tabs-window v-model="tab">
						<v-tabs-window-item v-for="hocKy in DSHocKy" :key="hocKy.id" :value="hocKy.id">
							<v-card>
								<v-divider></v-divider>
								<v-expansion-panels v-model="GroupPanel" flat multiple>
									<v-expansion-panel>
										<v-expansion-panel-title class="bg-blue-lighten-5" expand-icon="mdi-menu-down">
										...
										<v-expansion-panel-text>
											<v-divider></v-divider>
											<v-row style="border: 1px solid #ddd" class="font-weight-medium rounded-t ">
												<v-col cols="6">
													<p class="ml-4">Môn học</p>
												</v-col>
												<v-col cols="3">
													<p class="ml-4 text-center">Mức đạt</p>
												</v-col>
												<v-col cols="2">
													<p class="ml-4 text-center">Điểm</p>
												</v-col>
												<v-col cols="1">
											...
											<v-expansion-panels>
												<v-expansion-panel v-for="(value, key) in groupKTKN" :key="key">
									...
									<v-expansion-panel>
										<v-expansion-panel-title class="bg-red-lighten-5" expand-icon="mdi-menu-down">
											<div class="d-flex align-center ga-4">
									...
									<v-expansion-panel>
										<v-expansion-panel-title class="bg-yellow-lighten-5"
									...
									<v-expansion-panel>
										<v-expansion-panel-title class="bg-brown-lighten-5" expand-icon="mdi-menu-down">
											<div class="d-flex align-center ga-4">
							...
							<v-card>
								<v-row class="ma-0">
									<v-col cols="12" class="pb-0 text-h6 text-primary">
									...
									<v-col cols="12">
										<v-row class="ma-0">
											...
											<v-col cols="6" lg="4" md="4" sm="6" class="pl-4 pb-0">
			...
			<div v-if="vueData.HocSinhSelected.CapID === 2 || vueData.HocSinhSelected.CapID === 3">
							...
							<v-expansion-panel-text>
								<div v-if="item.id == 1">
									...
									<div class="d-flex flex-column px-2">
										<v-row style="border: thin solid #e0e0e0; border-radius: 5px;"
											class="mt-2 pa-3">
											<!-- <v-col cols="12">
								...
								<div v-if="item.id == 2">
									...
									<v-row style="border: thin solid #e0e0e0; border-radius: 5px;" class="mt-2 pa-3">
										<!-- <v-col cols="12">
											<v-icon color="blue" class="mr-1">mdi-chart-bar</v-icon>
				...
				<div v-else>
					...
					<v-tabs-window v-model="tab">
						<v-tabs-window-item v-for="hocKy in DSHocKy" :key="hocKy.id" :value="hocKy.id">
							<v-expansion-panels flat selected-class='bg-primary' variant="default">
								...
								<v-expansion-panel readonly :expandIcon="null" :expanded="false"
									v-for="item in dataCN_C2_C3.Diem" v-if="hocKy.id === 3">
									<v-expansion-panel-title
										style="background-color: #e7f1ff; padding: 12px; border-radius: 0;">
										<!-- <img src="/_cdn/lhbs-lms/icon_hk.png" height="25" class="mr-2" /> -->
										<div class="d-flex justify-space-between align-center w-100">
...
export default {
	...
	data() {
		...
		// 		{
		// 			id: 1,
		// 			name: `${t('message.midSesemter')} 1`
		// 		},
		// 		{
		// 			id: 2,
		// 			name: `${t('message.finalSesemter')} 1`
		// 		},
		// 		{
		// 			id: 3,
		// 			name: `${t('message.midSesemter')} 2`
		// 		},
		// 	]
		// 	// if (vueData.user.UserID === 'P21100117') {
		// 	// 	DSHocKy.push({
		// 	// 		id: 4,
		// 	// 		name: `${t('message.finalSesemter')} 2`
		// 	// 	})
		// 	// }
		// } else {
		// 	DSHocKy = [{
		...
		// 	{
		// 		id: 3,
		// 		name: `${t('message.allYear')}`
		// 	}]
		// }

		// let Semester = ''
		// if (vueData.HocSinhSelected.CapID === 1) Semester = 3//4
		// else Semester = localStorage.getItem('Semester') ? JSON.parse(localStorage.getItem('Semester')) : 1
	...
	async mounted() {
		// this.$nextTick(() => {
		// 	const tabEl = document.querySelector(`button[value="${4}"]`);
		// 	const tabContainer = document.querySelector('#Tab_C1 .v-slide-group__content');

		// 	if (tabEl && tabContainer) {
		// 		tabContainer.scrollTo({
		// 			left: tabEl.offsetLeft - tabContainer.offsetLeft,
		// 			behavior: 'smooth'
		// 		});
		// 	}
		// });
		...
		await this.loadHocSinhKQHT('HK' + this.tab)
	...
	watch: {
		...
		tab: async function (tab) {
			...
			if (vueData.HocSinhSelected.CapID === 1) this.DSMonHoc_NhomDiem = await this.loadHocSinhKQHT('HK' + tab);
		}
	},
	methods: {
		loadCongBoBangDiem() {
					...
					res => {
						...
						if (vueData.HocSinhSelected.CapID === 1) {
							this.DSHocKy = data.map(x => {
								let name = ''
								// if (x.KyDanhGia === 1) name = `${this.$t('message.midSesemter')} 	1`
								// if (x.KyDanhGia === 2) name = `${this.$t('message.finalSesemter')} 	1`
								// if (x.KyDanhGia === 3) name = `${this.$t('message.midSesemter')} 	2`
								// if (x.KyDanhGia === 4) name = `${this.$t('message.finalSesemter')} 	2`
		...
		loadHocSinhKQHT(code) {
			return new Promise(async resolve => {
				ajaxCALL('solienlac/LMS_GetBangDiem', {
					...
					console.log(vueData.HocSinhSelected)
					if (vueData.HocSinhSelected.CapID === 1) {
						this.groupKTKN = this.fn_groupKTKN(res.data[0])
						this.groupNLR = this.fn_groupNLR(res.data[0])
						this.groupPC = this.fn_groupPC(res.data[0])
						this.groupNLC = this.fn_groupNLC(res.data[0])
						this.ChuyenCan_KQCN_C1 = {
							khongphep: res.data[0].khongphep,
							cophep: res.data[0].cophep,
							ditre: res.data[0].ditre,
							khoiid: res.data[0].khoiid,
							khenthuong: res.data[0].khenthuong,
							renluyenhe: res.data[0].renluyenhe,
						}
						...
						ajaxCALL('lms/LMS_OutSLL_GetKetQuaHocTap_ByHocSinhID', {
							...
							console.log('uniqueMonHoc', [...new Set(data.map(x => x.MonHocName))])
							...
							console.log('LMS_', monHocFilter_HocSinh)
							...
							console.log("groupKTKN", this.groupKTKN)
							// console.log("groupNLR", this.groupNLR)
							// console.log("groupPC", this.groupPC)
							// console.log("groupNLC", this.groupNLC)
