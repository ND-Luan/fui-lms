		...
		<v-card-text>
			<v-row>
				...
				<v-col class="d-flex ga-2">
					...
					<v-btn color="primary" variant="outlined" prepend-icon="mdi-file-excel" @click="onExportExcel">
						Xuất excel
					</v-btn>
				</v-col>
			</v-row>
...
<script>
	export default {
		...
		methods: {
			...
			getDataByMonHocID(allHocKiData, chiTieuByMonHocID, MonHocID, MonHocCode, NienKhoa, hocKiSoSanhObj, dataCacNamTruoc) {
				...
				for (const khoiID of khois) {
					...
					for (const hk of this.DSHocKi) {
						...
						row[`${MonHocCode}TileChuaHoanThanh_${hk.textValue}`] = this.getMonValue(dataHK, khoiID, MonHocCode, 'TileChuaHoanThanh')
			...
			onExportExcel() {
				exportExcel(this.DSMonHoc, this.DSHocKi, this.HocKiSoSanh, `NangLuc_TongHop_NK${this.NienKhoa}.xlsx`)
			}
		}
	}
</script>