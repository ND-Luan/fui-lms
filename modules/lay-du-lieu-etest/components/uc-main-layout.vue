	...
	export default {
		...
		methods: {
			...
			async onQuanLiKiThiApply({ exam, lopID, skillMapping, mappingField, studentScores }) {
				...
				// Xóa entries cũ của tab này trước để tránh duplicate/tenNhom sai
				const withoutOldTab = Object.fromEntries(
					Object.entries(this.changedCells).filter(([, v]) => v.wsIdx !== targetIdx)
				)
				this.changedCells = { ...withoutOldTab, ...changedBuffer }
