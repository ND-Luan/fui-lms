		...
		<v-card-title class="px-0">
			<v-row dense class="px-0">
				...
				<v-col cols="12" md="3" lg="2">
					<v-btn variant='outlined' prepend-icon="mdi-chart-line" color="primary"
						@click="ThongKe_TiLeDat_TA2_GET()" :disabled='!HocKyValue || List_LopID.length === 0'>Xem thống
						kê</v-btn>
				</v-col>
			</v-row>
		</v-card-title>
		...
		methods: {
			async ThongKe_TiLeDat_TA2_GET() {
				...
				if (this.HocKyValue && this.KhoiID && this.List_LopID) {
					...
					ajaxCALL('lms/ThongKe_TiLeDat_TA2_GET', { ...params, HocKi }, res => {
						const $this = this
						const _dataSheet = []
