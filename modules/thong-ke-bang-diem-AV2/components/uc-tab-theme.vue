<template>
    ...
    <!-- Biểu đồ Tiếng Anh 2 -->
    <v-card class="mt-5 pa-4 elevation-1 rounded-lg" v-if="HocSinhID && KhoiID == 12">
        <v-card-title class="text-primary text-subtitle-1 font-weight-medium pb-2">
            📘 Thống kê điểm IELTS
        </v-card-title>
        <v-card-text>
            <v-row dense>
                <v-col cols="12" md="6">
                    <uc-chart-apex :options="optionsChart1_IELTS" />
                </v-col>
                <v-col cols="12" md="6">
                    <uc-chart-apex :options="optionsChart2_IELTS" />
                </v-col>
            </v-row>
        </v-card-text>
    </v-card>
...
export default {
    ...
    methods: {
        ...
        async ThongKeDiemTA2_ByHocSinh() {
            return new Promise(resolve => {
                ajaxCALL('lms/ThongKeDiemTA2_ByHocSinh',
                    ...
                    res => {
                        this.DataChart2 = res.data[0]
                        this.DataChart1_IELTS = res.data[1]
                        const DSKyHoc = ['S1_Mid_TA2', 'S1_Final_TA2', 'S2_Mid_TA2', 'S2_Final_TA2']
                        const DSKyHocIELTS = ['S1_Mid_IELTS', 'S1_Final_IELTS', 'S2_Mid_IELTS', 'S2_Final_IELTS']
                        let dataHandle = []
                        let dataHandleIELTS = []
                        ...
                        for (let ky of DSKyHocIELTS) {
                            let obj = this.DataChart1_IELTS.find(item => item.MaNhomCotDiem == ky)
                            if (obj) {
                                dataHandleIELTS.push(obj.KetQuaDanhGia_VI)
                            }
                            else {
                                dataHandleIELTS.push(null)
                            }
                        }
                        this.optionsChart1_IELTS = {
                            ...this.optionsChart2,
                            series: [
                                {
                                    name: 'Point',
                                    data: dataHandleIELTS
                                }
                            ],
                            xaxis: {
                                categories: ['Giữa HK1', 'Cuối HK1', 'Giữa HK2', 'Cuối HK2'],
                            ...
                            yaxis: {
                                min: Math.min(...dataHandleIELTS) - 2,
                                max: 10,
                                stepSize: 0.5
                            }
                        }
        ...
        async DashboardDiemTrungBinhTheoTheme_Get() {
            return new Promise(resolve => {
                ajaxCALL('lms/DashboardDiemTiengAnhHocKy',
                    {
                        HocSinhID: this.HocSinhID,
                        KhoiID: this.KhoiID,
                        LopID: this.LopID,
                    ...
                    res => {
                        ...
                        const DSKyHoc_IELTS = ['S1_Mid_IELTS', 'S1_Final_IELTS', 'S2_Mid_IELTS', 'S2_Final_IELTS']
                        const DSCotDiem_IELTS = ['IELTS_Listening_Conv', 'IELTS_Speaking_Conv', 'IELTS_Reading_Conv', 'IELTS_Writing_Conv', 'IELTS_Band_Conv']
                        this.DataChart3 = res.data[0]
                        this.DataChart2_IELTS = res.data[1]
                        let dataHandle = []
                        let dataHandleIELTS = []
                        for (let ky of DSKyHoc) {
                            let obj = {
                        ...
                        for (let ky of DSKyHoc_IELTS) {
                            let obj = {
                                KyHoc: ky,
                                data: []
                            }
                            for (let cotDiem of DSCotDiem_IELTS) {
                                let objFind = this.DataChart2_IELTS.filter(item => item.MaNhomCotDiem == ky).find(obj => obj.MaCotDiem.includes(cotDiem))
                            ...
                            dataHandleIELTS.push(obj)
                        }
                        console.log('this.DataChart2_IELTS', this.DataChart2_IELTS)
                        console.log('dataHandleIELTS', dataHandleIELTS)


                        this.optionsChart2_IELTS = {
                            ...this.optionsChart3,
                            series: [
                                {
                                    name: 'Giữa HK1',
                                    data: dataHandleIELTS[0].data
                                },
                                {
                                    name: 'Cuối HK1',
                                    data: dataHandleIELTS[1].data
                                },
                                {
                                    name: 'Giữa HK2',
                                    data: dataHandleIELTS[2].data
                                },
                                {
                                    name: 'Cuối HK2',
                                    data: dataHandleIELTS[3].data
                                }
                            ],
                            xaxis: {
                                categories: ['Nghe', 'Nói', 'Đọc', 'Viết', 'Trung bình'],
