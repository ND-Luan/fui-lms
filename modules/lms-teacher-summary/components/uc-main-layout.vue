	...
	<Global>
		<template #header>
			<v-card>
				<v-card-title>
					{{ TitlePage }}
					<v-chip size="small" color="primary" variant="tonal" prepend-icon="mdi-calendar" class="ml-2">
				...
				<v-card-text>
					<v-row align="center">
						<v-col>
							<v-select v-model="CapID" label="Chọn cấp học" :items="DSCap" item-title="title"
								item-value="value" />
						</v-col>
						<v-col>
							<uc-date-range-picker v-model="dateRange" />
							<v-btn color="success" variant="tonal" prepend-icon="mdi-microsoft-excel" @click="onExport">
								Xuất Excel
							</v-btn>
						</v-col>
					</v-row>
		...
		<!-- Tabs -->
		<v-card rounded="0" elevation="0" border="0">
